...
def str_to_bool(s):
    ...
    return s.strip().lower() in ("yes", "true", "1")  # <-- ADDED
...
def main():
    ...
    # File format is like: Language,Typing,Reflection,Year,PointerArithmetic   # <-- UPDATED
    in_file.readline()
    ...
    for line in in_file:
        ...
        reflection = str_to_bool(parts[2])          # CHANGED
        pointer_arithmetic = str_to_bool(parts[4])  # ADDED
        # Construct a ProgrammingLanguage object using the elements
        # year should be an int
        language = ProgrammingLanguage(parts[0], parts[1], reflection, int(parts[3]), pointer_arithmetic)  # <-- UPDATED
...
def using_namedtuple():
    ...
    Language = namedtuple('Language', 'name, typing, reflection, year, pointer_arithmetic')  # <-- UPDATED
